...
NexusDB is an open-source graph-vector database built from scratch in Rust, designed specifically for AI applications, agents, and Retrieval-Augmented Generation (RAG) systems.
...
NexusDB consolidates these concerns into a single database that understands the interconnected nature of AI data: entities have both properties (traditional data), relationships (graph structure), and semantic meaning (vector embeddings).
...
See the [Getting Started Guide](./getting-started.md) for installation and first steps, or explore the [Key Features](../features/README.md) to understand what makes NexusDB unique.